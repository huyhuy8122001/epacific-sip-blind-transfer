...
router.post("/", (req, res) => {
  ...
  try {
    ...
    app
      .say({ text: "Hi there.  Please hold while we connect your call" })
      .dial({
        answerOnBridge: true,
        ...
        target: [
          {
            type: "user",
            name: `${req.body.to}@pmice842266.ew.ccall.vn`,
          },
        ],
      });
...
router.post("/refer", async (req, res) => {
  ...
  try {
    ...
    const transferringChild =
      !parent_call_sid || referring_call_sid === parent_call_sid;
  ...
  try {
    ...
    app.dial({
      ...
      target: [
        {
          type: "user",
          name: `${number}@pmice842266.ew.ccall.vn`,
        },
      ],
    });
    res.status(200).json(app);
    ...
    callStatusEmitter.on(
      req.body.call_sid,
      _onNewCallStatusChange.bind(
        this,
        logger,
        client,
        req.body.call_sid,
...
const _onNewCallStatusChange = async (
  logger,
  client,
  call_sid,
  referringCallSid,
  ...
  if (sip_status >= 200) {
    ...
    try {
      await client.calls.update(referringCallSid, {
        call_hook: "/dial/hangup",
      });
    } catch (err) {
